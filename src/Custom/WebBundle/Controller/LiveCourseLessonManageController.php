...
{
    const LIVE_PROVIDER_UCLOUD = 100;

    public function liveStatusAction(Request $request,$courseId,$lessonId)
    {
        $rq = array('code' => 0,'message' => '请求成功');
        $classId = '900'.$courseId.$lessonId;//TODO:change the rule generating classId
        try
        {
            ...
            if ($cache_data)
            {
                return $this->createJsonResponse($rq);
            }
            else
            {
                $rq['code'] = '1';
...
//            }
            ...
            $liveLesson['mediaId']      = 0;
            $liveLesson['liveProvider'] = self::LIVE_PROVIDER_UCLOUD;
        ...
        return $this->render('CustomWebBundle:LiveCourseLessonManage:live-lesson-modal.html.twig', array(
