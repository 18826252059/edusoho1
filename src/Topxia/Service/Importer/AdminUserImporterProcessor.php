...
class AdminUserImporterProcessor implements ImporterProcessor
{
    protected $necessaryFields = array('nickname' => '手机号','subType' => '是否为培育对象');//手机号（如果excel有其他字段，在array里面增加）
    protected $objWorksheet;
    protected $rowTotal = 0;
    protected $colTotal = 0;
    protected $excelFields = array();
    protected $excelExample = 'bundles/classroom/example/user_import_example.xls';
    protected $validateRouter = 'classroom_manage_student_import';
    protected $importingRouter = 'lpmasReg_test';//上传路径
    ...
    public function excelAnalyse($file)
    {
        ...
        {
            $fieldTitle = $objWorksheet->getCellByColumnAndRow($col, 2)->getValue();//只读第2行
    ...
    public function validExcelFieldValue($userData,$row,$fieldCol)
    {
        ...
        if(strlen($userData['nickname']) != 11) {
            $errorInfo = "第 ".$row."行".$fieldCol["nickname"]." 列 手机号码不正确，请检查。";
        }
