...
class UserServiceImpl extends BaseService implements UserService
{
    ...
    public function register($registration, $type = 'default')
    {
        ...
        elseif ($type == 'lpmas' || $type == 'longping')
        {
            $user['salt'] = '';
            $user['password'] = '';
            $user['setup'] = 0;
            $user['smallAvatar'] = empty($registration['smallAvatar']) ? '' : $registration['smallAvatar'];
            $user['mediumAvatar'] = empty($registration['mediumAvatar']) ? '' : $registration['mediumAvatar'];
            $user['largeAvatar'] = empty($registration['largeAvatar']) ? '' : $registration['largeAvatar'];
            $user['verifiedMobile'] = empty($registration['verifiedMobile']) ? '' : $registration['verifiedMobile'];;
        }
        else
        {
            $user['salt'] = '';
...
//        $this->getDispatcher()->dispatch('user.service.registered', new ServiceEvent($user));
    ...
    public function bindUser($type, $fromId, $toId, $token)
    {
        ...
        $types = array_keys(OAuthClientFactory::clients());
        $types = array_merge($types, array('discuz', 'phpwind', 'lpmas','longping'));
...
class UserSerialize
{
    ...
    //add by MZ
    protected function getClientUsingLogDao()
    {
        return $this->createDao('User.ClientUsingLogDao');
    }
}
